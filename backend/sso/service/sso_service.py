...
def user_register(db:Session,user:RegisterUser)->JSONResponse:
    #check username, email, phone number exist in Bloom filter
    if redis_client.abloom_exists("bf:sso:username",user.username):
        raise BaseServiceException(code=400,msg= "username exists!")
    if redis_client.abloom_exists("bf:sso:email",user.email):
    ...
    try:
        ...
        redis_client.abloom_remove("bf:sso:username",user.username)
        redis_client.abloom_remove("bf:sso:email",user.email)
